...
// MARK: BufferList 
    ...
    /**
    This class provides an implementation of a buffer than can be added to and taken from in chunks. Data is always added to the end of the buffer (using BufferList.append(...) and taken out of the buffer (using BufferList.fill(...)) from the beginning towards the end. The location indicating where the next chunk of data will be taken from is maintained, this location can then be reset to enable data to be taken out of the buffer from the beginning again.
    ...
    writeBuffer.append(bytes: UnsafePointer<UInt8>(firstArrayOfNumbers), length: MemoryLayout<UInt8>.size * firstArrayOfNumbers.count)
    ...
    writeBuffer.append(bytes: UnsafePointer<UInt8>(secondArrayOfNumbers), length: MemoryLayout<UInt8>.size * secondArrayOfNumbers.count)
    ...
    let count = writeBuffer.fill(buffer: UnsafeMutableRawPointer(mutating: finalArrayOfNumbers).assumingMemoryBound(to: UInt8.self), length: ((MemoryLayout<UInt8>.size) * finalArrayOfNumbers.count))
